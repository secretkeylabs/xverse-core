...
import { getSigningDerivationPath, parsePsbt, psbtBase64ToHex, signBip340, signPsbt } from '../../transactions/psbt';
...
describe('Bitcoin PSBT tests', () => {
  it('can get signing BTC derivation path from address', async () => {
    ...
    const wrappedSegwitPath = getSigningDerivationPath(accounts, wrappedSegwitAddress1, 'Mainnet');

    expect(wrappedSegwitPath).eq(`m/49'/0'/0'/0/0`);

    const testnetWrappedSegwitPath = getSigningDerivationPath(accounts, wrappedSegwitAddress1, 'Testnet');

    expect(testnetWrappedSegwitPath).eq(`m/49'/1'/0'/0/0`);

    const nativeSegwitPath = getSigningDerivationPath(accounts, nativeSegwitAddress2, 'Mainnet');

    expect(nativeSegwitPath).eq(`m/84'/0'/0'/0/1`);

    const taprootPath = getSigningDerivationPath(accounts, taprootAddress3, 'Mainnet');
  ...
  it('can sign taproot psbt', async () => {
    ...
    expect(tx.inputsLength).eq(2);
    expect(tx.outputsLength).eq(2);
  });
  ...
  it('can sign native segwit psbt with multi input + sighash', async () => {
    ...
    expect(tx.inputsLength).eq(3);
    expect(tx.outputsLength).eq(3);
    expect(tx.getOutput(0).amount === 5000n);
    expect(tx.getOutput(1).amount === 6000n);
    expect(tx.getOutput(2).amount === 2500n);
    expect(signedTx).eq(
  ...
  it('can sign message BIP340 for taproot address', async () => {
    ...
    const signature = await signBip340(testSeed, accounts, taprootAddress, messageHash);

    const verify = await secp256k1.schnorr.verify(signature, hex.decode(messageHash), hex.decode(publicKey));
  ...
  it('can parse PSBT with nonWitnessUtxo', async () => {
    const btcAddress = 'bc1qf8njhm2nj48x9kltxvmc7vyl9cq7raukwg6mjk';
    const taprootAddress = 'bc1ps5a0retlxrjdly05nwwvmu8ek69j8mhyy8hl0hmeclmq66r0p4rq083nr0';
    ...
    const psbt =
      '';
    ...
    expect(parsedPsbt.inputs.length).eq(3);
    expect(parsedPsbt.outputs.length).eq(2);
    expect(parsedPsbt.inputs[0].value).eq(2500n);
    expect(parsedPsbt.inputs[1].value).eq(6000n);
    expect(parsedPsbt.inputs[2].value).eq(19681n);
    expect(parsedPsbt.outputs[0].amount).eq(6000n);
    expect(parsedPsbt.outputs[1].amount).eq(16681n);
    expect(parsedPsbt.outputs[0].address).eq('37qg72pj86wvAEYh8TK42g7ZyHu5eytdRH');
    expect(parsedPsbt.outputs[1].address).eq('bc1ps5a0retlxrjdly05nwwvmu8ek69j8mhyy8hl0hmeclmq66r0p4rq083nr0');
    expect(parsedPsbt.netAmount).eq(14181n);
    expect(parsedPsbt.fees).eq(5500n);
  });
  ...
  it('can convert PSBT from base64 to hex', async () => {
    const psbtBase64 =
      '';
    const expectTxHex =
      '';
    const txHex = psbtBase64ToHex(psbtBase64);
    expect(txHex).eq(expectTxHex);
  });
});
